...
def orthogonal_polynomial(
    ...
    phi = np.array(
        [[[0.0 for k in range(dm)] for i in range(pop_size)] for j in range(sites)]
    )
    ...
    phi2 = np.array(
        [
            [
                [[[0.0 for k in range(dm)] for i in range(dm)] for j in range(pop_size)]
    ...
    phi2m = np.array(
        [
            [[[0.0 for k in range(dm)] for i in range(dm)] for l in range(sites)]
            for m in range(sites)
        ]
    )
    P = np.array(
        [[[0.0 for z in range(dm)] for j in range(pop_size)] for i in range(sites)]
    )
    cov = np.array(
        [
            [[[0.0 for z in range(dm)] for i in range(dm)] for j in range(sites)]
    ...
    if dm == 3:
        if molecule == "protein_pnp":
            for protein_alphabet_index_polar in range(len(PROTEIN_ALPHABETS_polar)):
                for protein_alphabet_index_nonpolar in range(
                    len(PROTEIN_ALPHABETS_nonpolar)
                ):
                    for i in range(pop_size):
                        for j in range(sites):
                            if (
                                seq[i][j]
                                == PROTEIN_ALPHABETS_polar[protein_alphabet_index_polar]
                            ):
                                phi[j][i][0] = 1.0
                            if (
                                seq[i][j]
                                == PROTEIN_ALPHABETS_nonpolar[
    ...
    if poly_order == "first":
        ...
        if precomputed == "True":
            ...
            # # # # Variance in P2i1
            varP1i1 = np.array(
                [[[0.0 for z in range(dm)] for i in range(sites)] for j in range(sites)]
            )
            for i in range(0, pop_size):  # individuals
                for j in range(sites):
                    for k in range(sites):
        ...
        if precomputed == "True":
            P1D = np.load(os.path.join(out_dir, naming + ".npz"))[naming + "_P1D"]
        else:
            P1D = np.array(
                [
                    [[0.0 for z in range(dm)] for i in range(pop_size)]
        ...
        if precomputed == "True":
            varP1D = np.load(os.path.join(out_dir, naming + ".npz"))[naming + "_varP1D"]
        else:
            # # # #variance in P1D
    ...
    if poly_order == "second":
        ...
        # sites, for individual i
        P1D = np.array(
            [[[0.0 for z in range(dm)] for i in range(pop_size)] for j in range(sites)]
        )
        for i in range(pop_size):  # indiv
            for j in range(sites):
                for k in range(sites):
        ...
        for i in range(pop_size):
            for j in range(sites):
                for k in range(sites):
                    if k != j:
                        cov2w1a[j][k] += (
                            sr.outer_general(Q2[j][k][i], P[0][i]) / pop_size
                        )
                        cov2w1b[j][k] += (
                    ...
                    if k != j:
                        for l in range(sites):
                            for m in range(sites):
                                if m != l:
                                    cov2w2[j][k][l][m] += (
                                        sr.outer_general(P2[j][k][i], P2[l][m][i])
                                        / pop_size
                                    )
