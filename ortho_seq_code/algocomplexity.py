...
while d != 0:
    ...
    if fx1d is not None:
        print("Difference of", str(fxd-fx1d), "computations (" + str(round(100*fxd/fx1d, 2)) + "% of previous value)")
    fx1d = d*p*(d*(s+1)+2*s+1)
    print("\nProjection will require", str(fxp), "computations.")
    if fx1p is not None:
        print("Difference of", str(fxp-fx1p), "computations (" + str(round(100*fxp/fx1p, 2)) + "% of previous value)")
