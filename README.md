...
--alphbt_input
```
Used to group amino acids/nucleotides together, or specify certain amino acids/nucleotides. For example, putting *ASGR* will tell the program to have 5 dimensions: one for each amino acid specified, and one for *n*, where every unspecified amino acid will be converted to *n*. Comma-separate amino acids/nucleotides to group them. For example, putting *AS,GR* will make the vectors 4-dimensional, one for *AS*, one for *GR*, one for every other amino acid, and one for *n*. Add a "-" at the end to group every other amino acid with *n*. For example, *AS,GR-* will be 3-dimensional, one for *AS*, one for *GR*, and one for everything else (including *n*). There are also built-in groups ("-" rule still applies, last specified group will be clustered with *n* if desired):
